...
bool AccProjector::setcompressMdlDim(
		int xdim, int ydim, int zdim,
		int inity, int initz,
		int maxr, int paddingFactor,int padsize,size_t sumdata,size_t *yoffsetdata)
{
	...
	DEBUG_HANDLE_ERROR(cudaMalloc( (void**) &mdlReal, sumalldata * sizeof(XFLOAT)));
	DEBUG_HANDLE_ERROR(cudaMalloc( (void**) &mdlImag, sumalldata * sizeof(XFLOAT)));
...
void AccProjector::initcompressMdl(XFLOAT *real, XFLOAT *imag)
{
#ifdef DEBUG_CUDA
	if (mdlXYZ == 0)
	{
...
#else
#ifdef CUDA
	DEBUG_HANDLE_ERROR(cudaMemcpy( mdlReal, real, sumalldata * sizeof(XFLOAT), cudaMemcpyHostToDevice));
	DEBUG_HANDLE_ERROR(cudaMemcpy( mdlImag, imag, sumalldata * sizeof(XFLOAT), cudaMemcpyHostToDevice));
#else
	std::complex<XFLOAT> *pData = mdlComplex;
    for(size_t i=0; i<mdlXYZ; i++) {
		std::complex<XFLOAT> arrayval(*real ++, *imag ++);
