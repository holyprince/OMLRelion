						...
						std::vector< AccPtrBundle > &bundleSWS)
{
	...
	for (long int ipart = 0; ipart < sp.nr_particles; ipart++)
	{
		...
		for (unsigned long iclass = sp.iclass_min; iclass <= sp.iclass_max; iclass++)
		{
			...
			//if(op.my_ori_particle < 100 || (op.my_ori_particle<1020 && op.my_ori_particle>1000) )
			//printf(" Particle id %d and partid %d and num is %ld \n",op.my_ori_particle,part_id,orientation_num);

/*			if(op.my_ori_particle ==0)
			{
				printf(" IMGXYZ : %d %d %d and padsize is %d and sp.nr_particles %d\n",op.local_Minvsigma2s[0].xdim,op.local_Minvsigma2s[0].ydim,op.local_Minvsigma2s[0].zdim,
						accMLO->bundle->backprojectors[iproj].mdlX,sp.nr_particles);
			...
			//1.hash GPU device malloc
			int imgx,imgy;
			imgx=op.local_Minvsigma2s[0].xdim;
			imgy=op.local_Minvsigma2s[0].ydim;

			int *flagdata;
			cudaMalloc((void **)&flagdata,sizeof(int)*1);

			int fullsize =imgx * imgy * 8 *sizeof(XFLOAT);
			if(op.my_ori_particle == 0)
				printf("CPU device size %d \n",fullsize);




			for(unsigned long imagenum=0;imagenum < orientation_num;imagenum++)
			{

				cudaMemset(flagdata,0,sizeof(int)*1);
#ifdef TIMBPS
			    gettimeofday (&tv1, &tz);
#endif
#ifdef FILTERSLICE
			    ...
			    int datacount=0;
			    runBackProjectKernel_filterslice(
			   						...
			   						(XFLOAT) op.sum_weight[ipart],
			   						~eulers[iclass],
			   						baseMLO->wsum_model.BPref[0].cpuindex.data,
			   						baseMLO->wsum_model.BPref[0].cpureal.data,
			   						baseMLO->wsum_model.BPref[0].cpuimag.data,
			   						baseMLO->wsum_model.BPref[0].cpuweight.data,
			   						op.local_Minvsigma2s[0].xdim,
			   						op.local_Minvsigma2s[0].ydim,
			   						op.local_Minvsigma2s[0].zdim,
			   						imagenum,
...
#ifdef TIMBPS
			    gettimeofday (&tv2, &tz);
			    baseMLO->wsum_model.timebp+= (float)(1000 * (tv2.tv_sec-tv1.tv_sec)+ (tv2.tv_usec-tv1.tv_usec)/1000.0);

			    part1 += (float)(1000 * (tv2.tv_sec-tv1.tv_sec)+ (tv2.tv_usec-tv1.tv_usec)/1000.0);
#endif

			    int flagcpu[1];
				cudaMemcpy(flagcpu,flagdata,sizeof(int)*1,cudaMemcpyDeviceToHost);
...
#ifdef PINMEM
			 		cudaMemcpy(baseMLO->wsum_model.BPref[0].cpuindex, accMLO->bundle->backprojectors[iproj].d_index,fullsize,cudaMemcpyDeviceToHost);
			    	cudaMemcpy(baseMLO->wsum_model.BPref[0].cpureal, accMLO->bundle->backprojectors[iproj].d_real,fullsize,cudaMemcpyDeviceToHost);
			    	cudaMemcpy(baseMLO->wsum_model.BPref[0].cpuimag, accMLO->bundle->backprojectors[iproj].d_imag,fullsize,cudaMemcpyDeviceToHost);
			    	cudaMemcpy(baseMLO->wsum_model.BPref[0].cpuweight, accMLO->bundle->backprojectors[iproj].d_weight,fullsize,cudaMemcpyDeviceToHost);
#else
			    	datacount = imgx * imgy * 8;

					cudaMemcpy(baseMLO->wsum_model.BPref[0].cpuindex.data, accMLO->bundle->backprojectors[iproj].d_index,datacount*sizeof(int),cudaMemcpyDeviceToHost);
					cudaMemcpy(baseMLO->wsum_model.BPref[0].cpureal.data, accMLO->bundle->backprojectors[iproj].d_real,datacount*sizeof(float),cudaMemcpyDeviceToHost);
					cudaMemcpy(baseMLO->wsum_model.BPref[0].cpuimag.data, accMLO->bundle->backprojectors[iproj].d_imag,datacount*sizeof(float),cudaMemcpyDeviceToHost);
					cudaMemcpy(baseMLO->wsum_model.BPref[0].cpuweight.data, accMLO->bundle->backprojectors[iproj].d_weight,datacount*sizeof(float),cudaMemcpyDeviceToHost);
...
#ifdef TIMBPS
				gettimeofday (&tv2, &tz);
				  baseMLO->wsum_model.timememcpy+= (float)(1000 * (tv2.tv_sec-tv1.tv_sec)+ (tv2.tv_usec-tv1.tv_usec)/1000.0);
				  part2+= (float)(1000 * (tv2.tv_sec-tv1.tv_sec)+ (tv2.tv_usec-tv1.tv_usec)/1000.0);
				...
				//#pragma omp parallel for num_threads(4)
					...
					{
#ifdef PINMEM
						size_t index = baseMLO->wsum_model.BPref[0].cpuindex[i];
				//#pragma omp atomic
						{
							baseMLO->wsum_model.BPref[0].data.data[index].real+=baseMLO->wsum_model.BPref[0].cpureal[i];
							baseMLO->wsum_model.BPref[0].data.data[index].imag+=baseMLO->wsum_model.BPref[0].cpuimag[i];
							baseMLO->wsum_model.BPref[0].weight.data[index]+=baseMLO->wsum_model.BPref[0].cpuweight[i];
						}
#else
						size_t index = baseMLO->wsum_model.BPref[0].cpuindex.data[i];
						if(index ==0) continue;
						baseMLO->wsum_model.BPref[0].data.data[index].real+=baseMLO->wsum_model.BPref[0].cpureal.data[i];
						baseMLO->wsum_model.BPref[0].data.data[index].imag+=baseMLO->wsum_model.BPref[0].cpuimag.data[i];
						baseMLO->wsum_model.BPref[0].weight.data[index]+=baseMLO->wsum_model.BPref[0].cpuweight.data[i];
#endif
					}
...
#ifdef TIMBPS
					 gettimeofday (&tv2, &tz);
					 baseMLO->wsum_model.timecalc += (float)(1000 * (tv2.tv_sec-tv1.tv_sec)+ (tv2.tv_usec-tv1.tv_usec)/1000.0);
					 part3+= (float)(1000 * (tv2.tv_sec-tv1.tv_sec)+ (tv2.tv_usec-tv1.tv_usec)/1000.0);
#endif
					cudaMemset(accMLO->bundle->backprojectors[iproj].d_index,0,fullsize);
					cudaMemset(accMLO->bundle->backprojectors[iproj].d_real,0,fullsize);
					cudaMemset(accMLO->bundle->backprojectors[iproj].d_imag,0,fullsize);
					cudaMemset(accMLO->bundle->backprojectors[iproj].d_weight,0,fullsize);
	//				cudaMemset(accMLO->bundle->backprojectors[iproj].d_count,0,fullsize);
				}
/*				else
				{
					baseMLO->wsum_model.count++;
				}*/
#ifdef TIMBPS
				baseMLO->wsum_model.countsum++;
#endif
			}
		...
		gettimeofday (&tv2, &tz);
		baseMLO->wsum_model.timebp+= (tv2.tv_sec-tv1.tv_sec)*1000000+(tv2.tv_usec-tv1.tv_usec);
		baseMLO->wsum_model.countsum+= orientation_num;

		if(op.my_ori_particle ==0)
		{
			printf(" 2. one particle time : %ld %d\n",baseMLO->wsum_model.timebp, baseMLO->wsum_model.countsum);
			fflush(stdout);
		}
#endif
		...
		if(op.my_ori_particle ==0)
		{
			printf(" 2. one particel time : %d %d\n",baseMLO->wsum_model.timebp, baseMLO->wsum_model.countsum);
		}
#endif
#ifdef TIMBPS
			 gettimeofday (&tv2, &tz);
...
/*
			if(op.my_ori_particle ==0)
			{
				...
				printf("orientation_num :%d \n",orientation_num);
				printf("IMG XYZ : %d %d %d \n",op.local_Minvsigma2s[0].xdim,op.local_Minvsigma2s[0].ydim,op.local_Minvsigma2s[0].zdim);
				int mdlxyz=accMLO->bundle->backprojectors[iproj].mdlX*accMLO->bundle->backprojectors[iproj].mdlY*accMLO->bundle->backprojectors[iproj].mdlZ;
				Image<float> datacpu(accMLO->bundle->backprojectors[iproj].mdlX,accMLO->bundle->backprojectors[iproj].mdlY,accMLO->bundle->backprojectors[iproj].mdlZ);

				cudaMemcpy(datacpu.data.data,accMLO->bundle->backprojectors[iproj].d_mdlReal,mdlxyz*sizeof(float),cudaMemcpyDeviceToHost);
				datacpu.setStatisticsInHeader();
				datacpu.setSamplingRateInHeader(1);
				FileName fn_root;
				fn_root.compose(baseMLO->fn_out+"_itreal", baseMLO->iter, "mrc", 3);
				...
				while(count<10)
				{
					if(datacpu.data.data[i]!=0)
					{
						printf("%d %f\n",i,datacpu.data.data[i]);
						count++;
					}
					i++;
					if(i==mdlxyz)
...
#ifdef TIMBPS
		if(op.my_ori_particle ==1 || op.my_ori_particle ==100 || op.my_ori_particle ==1000)
		{
			printf("Particle i time : %f %f %f \n",part1,part2,part3);
			fflush(stdout);
		}
#endif
