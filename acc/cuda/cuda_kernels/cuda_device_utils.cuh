...
__device__ inline void cuda_atomic_add(int* address, int value)
{
	atomicAdd(address,value);
}
#endif
...
#ifdef COMFORGPU
static inline
__device__ XFLOAT no_tex3D_compress(XFLOAT* mdl, XFLOAT xp, XFLOAT yp, XFLOAT zp, int mdlX, int mdlXY, int mdlInitY, int mdlInitZ,
		size_t datarange,size_t *g_yoffsetdata,int pad_size)
{
	int x0 = floorf(xp);
	XFLOAT fx = xp - x0;
	int x1 = x0 + 1;
	...
	XFLOAT d000=0;
	size_t datacur;
	if(x0 * x0 + (y0+mdlInitY) * (y0+mdlInitY) + (z0+mdlInitZ) * (z0+mdlInitZ) < datarange )
	{
		datacur=g_yoffsetdata[(z0)*pad_size+(y0)]+(x0);
		d000 = mdl[datacur];
	}
	XFLOAT d001=0;
	if(x1 * x1 + (y0+mdlInitY) * (y0+mdlInitY) + (z0+mdlInitZ) * (z0+mdlInitZ) < datarange )
	{
		datacur=g_yoffsetdata[(z0)*pad_size+(y0)]+(x1);
		d001 = mdl[datacur];
	}
	XFLOAT d010=0;
	if(x0 * x0 + (y1+mdlInitY) * (y1+mdlInitY) + (z0+mdlInitZ) * (z0+mdlInitZ) < datarange )
	{
	...
	//////////
	XFLOAT d011=0;
	if(x1 * x1 + (y1+mdlInitY) * (y1+mdlInitY) + (z0+mdlInitZ) * (z0+mdlInitZ) < datarange )
	{
		datacur=g_yoffsetdata[(z0)*pad_size+(y1)]+(x1);
		d011 = mdl[datacur];
	}
	XFLOAT d100=0;
	if(x0 * x0 + (y0+mdlInitY) * (y0+mdlInitY) + (z1+mdlInitZ) * (z1+mdlInitZ) < datarange )
	{
		datacur=g_yoffsetdata[(z1)*pad_size+(y0)]+(x0);
		d100 = mdl[datacur];
	}
	XFLOAT d101=0;
	if(x1 * x1 + (y0+mdlInitY) * (y0+mdlInitY) + (z1+mdlInitZ) * (z1+mdlInitZ) < datarange )
	{
		datacur=g_yoffsetdata[(z1)*pad_size+(y0)]+(x1);
		d101 = mdl[datacur];
	}
	XFLOAT d110=0;
	if(x0 * x0 + (y1+mdlInitY) * (y1+mdlInitY) + (z1+mdlInitZ) * (z1+mdlInitZ) < datarange )
	{
		datacur=g_yoffsetdata[(z1)*pad_size+(y1)]+(x0);
		d110 = mdl[datacur];
	}
	XFLOAT d111=0;
	if(x1 * x1 + (y1+mdlInitY) * (y1+mdlInitY) + (z1+mdlInitZ) * (z1+mdlInitZ) < datarange )
	{
