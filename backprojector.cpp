...
//	fftoffset[1] = Ndim[0] * (Ndim[1] / 2);
	...
	else
	{
		...
		for (int iter = 0; iter < max_iter_preweight; iter++)
		{
			...
			for (int i = 0; i < GPU_N; i++) {
				cudaSetDevice(dataplan[i].devicenum);
				vector_Multi_layout_mpi(d_blockone[i],d_blocktwo[i],dataplan[i].d_Data+dataplan[i].selfoffset,dataplan[i].realsize,Fconv.xdim,pad_size,offsetZ[i]);
...
//==============================================================================add  GPU version
	...
	else
	{
		...
		for (int iter = 0; iter < max_iter_preweight; iter++)
		{
			...
			for (int i = 0; i < GPU_N; i++) {
				cudaSetDevice(plan[i].devicenum);
				vector_Multi_layout_mpi(d_blockone[i],d_blocktwo[i],plan[i].d_Data, plan[i].realsize,Fconv.xdim,pad_size,offsetZ[i]);
